...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (value === '--channel') args.channel = argv[++i];
    else if (value === '--headed') args.headed = true;
    else if (value === '-h' || value === '--help') {
      console.log('Usage: inspect-threejs-canvas.mjs [--url URL] [--out DIR] [--mobile] [--wait MS] [--channel chrome] [--headed]');
      process.exit(0);
    } else {
...
async function main() {
  ...
  await page.waitForFunction(() => {
    ...
    return (canvas instanceof HTMLCanvasElement && canvas.clientWidth > 0 && canvas.clientHeight > 0)
